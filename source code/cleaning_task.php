...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_status"])){
    ...
    if($status !== "completed"){
        ...
        $sql = "UPDATE cleaning_schedule SET status = ? WHERE id = ? AND staff_id = (SELECT staff_id FROM users WHERE id = ?)";
        if($stmt = mysqli_prepare($conn, $sql)){
            mysqli_stmt_bind_param($stmt, "sii", $status, $schedule_id, $_SESSION["id"]);
...
<head>
    <meta charset="UTF-8">
    <title>My Cleaning Tasks - Hostel Management System</title>
